...
class PlayerStats:
    """
    This class will get the player's statistics of the past ten games. The statistics will be in the form of a list
    of lists. Each list will contain the statistics of the past ten games. The statistics will be in the order of [
    ...
    def __init__(self, player_id: int, season: str) -> None:
        ...
        lists. Each list will contain the statistics of the past ten games. The statistics will be in the order of [
        rebounds, assist, steal, blocks, points]. The most recent game will be the first index of the list.
        """
    ...
    def get_past_ten_stats(self, season:str) -> None:
        ...
        order of [rebounds, assist, steal, blocks, points]. The most recent game will be the first index of the list.
        """
        ...
        # after filtering the data needed, cut the amount of data to the recent 10, then re-organize it for easier
        # understanding
...
def retrieve_past_ten(stats: list[list[int]]) -> list[list[int]]:
    """
    This function will return the past ten games data in the raw form from the API/web scraping. The data will be
    in the form of a list of lists. Each list will contain the statistics of the past ten games. The statistics
    will be in the order of [rebounds, assist, steal, blocks, points]. The most recent game will be the first
    index of the list.
    """
...
def find_player_stats(past_ten_data: list[list[int]]) -> dict[str, list[int]]:
    """
    This function will get a dictionary with the key of statistics category and value of statistics list given
    the past ten games data in the raw form from the API/web scraping. After getting the raw form data,
    it should return something like {'Rebounds': [10, 8, 1, 3, 10, 6, 5, 9, 2, 4], 'Assist': [1, 1, 0, 1, 0, 2,
    1, 1, 0, 0], 'Steal': [2, 2, 1, 1, 0, 0, 1, 2, 0, 0], 'Blocks': [5, 0, 0, 1, 0, 1, 1, 1, 0, 0], 'Points': [
    17, 10, 2, 2, 8, 11, 5, 16, 2, 2]}
    """
...
def retreive_player_id() -> int:
    """
    This function will return the player id given the player name. The player name will be given by the user. The
    player id will be used to get the past ten games data from the API.
    """
...
def get_betting_category(all_stats: dict[str, list[int]]) -> str:
    ...
    k:statistic category(ex, points, rebounds, etc) and v:list of past statistics(ex. 20,23,23,20,18), it will print
    out all the options for the user, and obtain which category the user wants to make a bet on.
    """
...
def analyze(category: str, sorted_data: dict[str, list[int]]) -> list[float]:
    """
    This function will return an integer value showing the degree of if the user should bet above or below the
    threshold. In the analysis, I will be using linear regression to see the trends and come up with a data based
    decision.
    """
...
def betting_decision(analysis_data: list[int], threshhold: float) -> None:
    """
    This function will print out the betting decision based on the analysis data. The analysis data will be a list of
    slope, y_intercept, R-squared value, Avg Residual diff. The threshold will be the value the user wants to bet on.
    """
